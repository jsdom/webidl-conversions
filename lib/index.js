...
function modulo(x, y) {
    ...
    // http://stackoverflow.com/a/4467559/3191
    return ((x % y) + y) % y;
}
...
function createIntegerConversion(bitLength, typeOpts) {
    ...
    return (V, opts) => {
        ...
        x = integerPart(x);
        x = modulo(x, Math.pow(2, bitLength));
        if (isSigned && x >= Math.pow(2, bitLength - 1)) {
...
exports["long long"] = createIntegerConversion(32, { unsigned: false });
exports["unsigned long long"] = createIntegerConversion(32, { unsigned: true });
